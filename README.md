...
- **검색(`/search`)** — 카테고리 탭(기술트렌드·주식·크립토) 아래 키워드 칩, 정확도순/최신순 정렬, 페이지네이션
...
Google 로 로그인하면 개인화 기능이 붙는다. 로그인하지 않아도 검색과 대시보드는 그대로 쓸 수 있다.
...
- **키워드 편집** — 대시보드 컬럼과 검색 카테고리를 직접 고친다. 언제든 기본값으로 되돌릴 수 있다
...
│  ├─ KeywordEditor.tsx   대시보드 컬럼과 검색 카테고리를 고치는 다이얼로그
...
기본 키워드는 `src/lib/constants.ts` 한 곳에서 바꾼다 — 대시보드 컬럼은 `DASHBOARD_KEYWORDS`,
검색 페이지의 카테고리·칩은 `KEYWORD_CATEGORIES`. 로그인한 사용자가 화면에서 고친 값은
`keyword_settings` 테이블에 저장되고, 저장한 적이 없으면 이 기본값으로 되돌아간다.
