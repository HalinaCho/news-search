...
내가 고른 키워드만 골라 네이버 뉴스를 훑어보는 작은 웹 앱이다.
편식은 원래 나쁜 습관이지만, 뉴스에서는 그게 요점이다.
...
- **중복 기사 묶기** — 같은 사건을 여러 매체가 받아쓴 기사를 한 장으로 접는다. 감추는 게 아니라 접는 것이라 "같은 내용 N곳 더 보기"로 언제든 펼친다
- 첫 화면은 첫 카테고리의 첫 키워드로 시작한다. 카테고리 순서를 바꾸면 첫 화면도 따라간다
...
- **안 읽은 것만 보기** — 결과 상단 토글로 이미 본 기사를 접는다. 상태는 URL(`?unread=1`)에 담겨 새로고침해도 유지된다
- **북마크(`/saved`)** — 카드 오른쪽 위 아이콘으로 담는다. 기사 내용을 통째로 복사해 두므로 검색 결과에서 밀려나도 계속 보인다
...
읽음 표시·북마크·키워드 편집을 쓰려면 필요하다. 건너뛰어도 검색 기능은 정상 동작한다.
...
└─ lib/
   ...
   ├─ dedupe.ts           같은 사건을 받아쓴 기사 묶기 (제목 유사도)
...
중복 묶기의 유사도 임계값(`src/lib/dedupe.ts`의 `SIMILARITY_THRESHOLD`)은 감이 아니라 측정으로 정했다.
제목에서 매체별 말머리(`[속보]`·`(종합)`)와 문장부호를 털어낸 뒤 글자 2개씩 잘라 Dice 계수로 비교하는데,
실제 검색 결과 8개 조합의 모든 제목 쌍을 뽑아보니 0.45 이상은 전부 같은 사건이었다. 표본이 좁아 **0.6**으로
여유를 뒀다. 처음 0.8로 뒀을 때는 실전에서 한 건도 안 잡혔으니, **바꿀 때는 반드시 실제 결과로 다시 재본다.**
접힌 기사는 카드에서 펼칠 수 있으므로 판정이 틀려도 사라지지는 않는다 — 이 안전장치가 있어서 낮출 수 있었다.
